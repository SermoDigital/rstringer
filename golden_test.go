...
func (i *Day) Set(s string) error {
	for j := 0; j < len(_Day_index)-1; j++ {
		if _Day_name[_Day_index[j]:_Day_index[j+1]] == s {
			*i = Day(j)
			return nil
		}
	}
...
func (i *Number) Set(s string) error {
	for j := 0; j < len(_Number_index)-1; j++ {
		if _Number_name[_Number_index[j]:_Number_index[j+1]] == s {
			*i = Number(j) - 1
			return nil
		}
	}
...
func (i *Gap) find(needle, haystack string, arr []uint8, off Gap) bool {
	for j := 0; j < len(arr)-1; j++ {
		if haystack[arr[j]:arr[j+1]] == needle {
			*i = Gap(j) - off
			return true
		}
	}
...
func (i *Gap) Set(s string) error {
	if i.find(s, _Gap_name_0, _Gap_index_0, 2) ||
		i.find(s, _Gap_name_1, _Gap_index_1, 5) ||
		i.find(s, _Gap_name_2, _Gap_index_2, 11) {
		return nil
	}
	return fmt.Errorf("invalid enum: %s", s)
}
...
func (i *Num) Set(s string) error {
	for j := 0; j < len(_Num_index)-1; j++ {
		if _Num_name[_Num_index[j]:_Num_index[j+1]] == s {
			*i = Num(j) - -2
			return nil
		}
	}
...
func (i *Unum) find(needle, haystack string, arr []uint8, off Unum) bool {
	for j := 0; j < len(arr)-1; j++ {
		if haystack[arr[j]:arr[j+1]] == needle {
			*i = Unum(j) - off
			return true
...
func (i *Unum) Set(s string) error {
	if i.find(s, _Unum_name_0, _Unum_index_0, 0) ||
		i.find(s, _Unum_name_1, _Unum_index_1, 253) {
		return nil
	}
	return fmt.Errorf("invalid enum: %s", s)
}
...
var _Prime_map_reverse = map[string]Prime{
	_Prime_name[0:2]:   2,
	_Prime_name[2:4]:   3,
	_Prime_name[4:6]:   5,
	_Prime_name[6:8]:   7,
...
func (i *Prime) Set(s string) error {
	if v, ok := _Prime_map_reverse[s]; ok {
		*i = v
		return nil
	}
	return fmt.Errorf("invalid enum: %s", s)
...
func TestGolden(t *testing.T) {
	for _, test := range golden {
		...
		g.generate(tokens[1], reverse)
		got := string(g.format())
		if got != test.output {
			writeDiff(tokens[1], got, test.output)
			t.Errorf("%s: got\n====\n%s====\nexpected\n====%s", test.name, got, test.output)
		}
	}
}

func writeDiff(prefix string, got, want string) {
	gout, err := os.Create(prefix + "1")
	if err != nil {
		panic(err)
	}
	defer gout.Close()
	wout, err := os.Create(prefix + "2")
	if err != nil {
		panic(err)
	}
	defer wout.Close()

	_, err = io.Copy(gout, strings.NewReader(got))
	if err != nil {
		panic(err)
	}
	_, err = io.Copy(wout, strings.NewReader(want))
	if err != nil {
		panic(err)
	}
}
